...
export const LoginRegister: React.FC<LoginRegisterProps> = ({ onLogin, onBackToHome, onNavigate }) => {
  ...
  const sendVerificationEmail = async () => {
    ...
    if (password !== confirmPassword) {
      showToastMessage('Passwords do not match. Please try again.');
      return;
    }
    // Validate terms agreement
    if (!agreedToTerms) {
      showToastMessage('You must agree to the Terms and Conditions to register.');
      return;
    }
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                {activeTab === 'register' ? (
                  <div className="grid grid-cols-2 gap-x-4 gap-y-3">
                    ...
                    <div className={`col-span-2 flex items-center border-b py-1.5 transition-all relative ${
                      confirmPassword && confirmPassword !== password
                        ? 'border-[#B34040] border-solid'
                        : 'border-[#EDE8DE] border-solid focus-within:border-[#2D5941]'
                    }`}>
                      <span className={`mr-2 ${confirmPassword && confirmPassword !== password ? 'text-[#B34040]' : 'text-[#8e8e93]'}`}>
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    {confirmPassword && confirmPassword !== password && (
                      <p className="col-span-2 text-[10px] text-[#B34040] -mt-1">Passwords do not match.</p>
                    )}
                  </div>
                ) : (
                  <>
                ...
                {activeTab === 'register' && (
                  <div className="flex items-start gap-2 pt-0.5">
                    ...
                    />
                    <label htmlFor="terms" className={`text-[10px] leading-relaxed ${agreedToTerms ? 'text-[#2D5941]' : 'text-[#8e8e93]'}`}>
                      I agree to the{' '}
                      <button
                        type="button"
