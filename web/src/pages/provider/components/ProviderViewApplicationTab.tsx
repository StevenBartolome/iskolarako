              ...
              <div className="p-4 rounded-2xl bg-[#F9F5EF]/60 border border-[#D9D2C5]/70 space-y-2">
                ...
                {application.payoutHistory && application.payoutHistory.length > 0 ? (
                    ...
                    <table className="w-full text-left text-xs">
                      <thead>
                        <tr className="bg-[#F9F5EF] text-[#6C6C70] border-b border-[#D9D2C5]/50">
                          <th className="px-2.5 py-1.5 text-[9px] font-bold uppercase">Date</th>
                          <th className="px-2.5 py-1.5 text-[9px] font-bold uppercase">Cycle / Semester</th>
                          <th className="px-2.5 py-1.5 text-[9px] font-bold uppercase">Amount</th>
                          <th className="px-2.5 py-1.5 text-[9px] font-bold uppercase">Status</th>
                        </tr>
                      </thead>
                        ...
                        {application.payoutHistory.map((payout: any, pIdx: number) => {
                          ...
                          return (
                            <tr key={payout.id || pIdx} className="hover:bg-[#F9F5EF]/40 transition-colors">
                              <td className="px-2.5 py-1.5 text-[#6C6C70] whitespace-nowrap">{payout.date || 'Recently'}</td>
                              <td className="px-2.5 py-1.5 font-medium text-[#1C1C1E]">{payout.cycleName || payout.semester || 'Intake Cycle'}</td>
                              <td className="px-2.5 py-1.5 font-mono font-bold text-[#2D5941] whitespace-nowrap">
                                ₱{Number(payout.amount || 0).toLocaleString()}
                              </td>
                              <td className="px-2.5 py-1.5 whitespace-nowrap">
                                {isRefunded ? (
                                  <span className="inline-flex items-center gap-1 text-[9px] font-bold px-1.5 py-0.5 rounded bg-rose-50 text-rose-800 border border-rose-200">
                                    Refunded
                                  </span>
                                ) : isReleased ? (
                  ...
                  <div className="p-3 bg-white rounded-xl border border-dashed border-[#D9D2C5] text-center text-xs text-[#6C6C70]">
                    No disbursements recorded for this scholar yet.
                  </div>
                )}
              </div>
            )}
