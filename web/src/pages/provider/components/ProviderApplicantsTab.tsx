            ...
            <div className="bg-[#1A3C2E] text-white px-5 py-3 rounded-2xl flex flex-wrap items-center justify-between shadow-lg text-xs animate-fade-in gap-3">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="inline-flex items-center gap-1.5 bg-[#2D5941] hover:bg-[#3D7355] text-white px-3.5 py-1.5 rounded-xl font-bold border-0 cursor-pointer shadow-sm text-xs transition-all"
                >
                  <GraduationCap className="w-3.5 h-3.5" /> Batch Approve ({selectedAppIds.length})
                </button>
                <button
                  type="button"
                  onClick={() => {
                  ...
                  className="inline-flex items-center gap-1.5 bg-purple-700 hover:bg-purple-800 text-white px-3 py-1.5 rounded-xl font-bold border-0 cursor-pointer text-xs transition-all"
                >
                  <ClipboardList className="w-3.5 h-3.5" /> Set For Exam ({selectedAppIds.length})
                </button>
                <button
                  type="button"
                  onClick={() => {
                  ...
                  className="inline-flex items-center gap-1.5 bg-sky-700 hover:bg-sky-800 text-white px-3 py-1.5 rounded-xl font-bold border-0 cursor-pointer text-xs transition-all"
                >
                  <BarChart2 className="w-3.5 h-3.5" /> Set Pending for Ranking ({selectedAppIds.length})
                </button>
                <button
                  type="button"
                  ...
                  <td colSpan={6} className="px-6 py-12 text-center text-[#8E8E93]">
                    <div className="flex flex-col items-center justify-center gap-2">
                      <div className="w-12 h-12 bg-[#EDE8DE] rounded-full flex items-center justify-center text-[#2D5941] text-xl mb-1">
                        <ClipboardList className="w-6 h-6 text-[#2D5941]" />
                      </div>
                  ...
                  <tr key={app.id} className="hover:bg-[#F9F5EF]/30 transition-colors">
                      ...
                      <div className="flex flex-col items-start gap-1">
                        ...
                        {app.isContinuingScholar && (
                          <span className="inline-flex items-center gap-1 text-[9px] font-bold px-2 py-0.5 rounded bg-[#EBF5EE] text-[#2D5941] border border-[#2D5941]/30">
                            <RotateCw className="w-2.5 h-2.5" /> Continuing Scholar
                          </span>
                        )}
                      </div>
                    </td>
                      ...
                      <div className="flex flex-col items-start gap-1">
                        ...
                        {isRenewal && (
                          <span className="inline-flex items-center gap-1 text-[9px] font-extrabold text-[#8C5216] bg-amber-100/90 border border-amber-300/60 px-2 py-0.5 rounded-full">
                            <RotateCw className="w-2.5 h-2.5" /> Semestral Renewal
                          </span>
                        )}
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-col items-start gap-1">
                        ...
                        >
                          {app.status === 'Rejected' ? (
                            <><XCircle className="w-3 h-3 text-[#B34040]" /> Rejected</>
                          ) : app.status === 'Waitlisted' ? (
                            <><Clock className="w-3 h-3 text-amber-800" /> Waitlisted</>
                          ) : (app.status === 'Under Review' || app.status === 'under_review') ? (
                            <><AlertTriangle className="w-3 h-3 text-amber-800" /> Under Review</>
                          ) : (app.status === 'Appealed' || app.status === 'appealed' || app.status === 'Appeals') ? (
                            <><ClipboardList className="w-3 h-3 text-purple-800" /> Appealed</>
                          ) : (app.status === 'Barred' || app.status === 'barred') ? (
                            <><Ban className="w-3 h-3 text-white" /> Barred</>
                          ) : (
                            app.status
                          )}
                        </span>
                          ...
                          <div>
                            {app.disbursement_mode === 'in_person_cash' ? (
                              <span className="inline-flex items-center gap-1 text-[9px] font-semibold text-slate-600 bg-slate-100 px-2 py-0.5 rounded">
                                <Banknote className="w-3 h-3 text-slate-500" /> Cash Payout
                              </span>
                            ) : app.paymentAccount ? (
                              <span className="inline-flex items-center gap-1 text-[9px] font-bold text-[#2D5941] bg-[#EBF5EE] border border-[#2D5941]/30 px-2 py-0.5 rounded" title={`${app.paymentAccount.bank_name} (${app.paymentAccount.account_number})`}>
                                <CreditCard className="w-3 h-3 text-[#2D5941]" /> {app.paymentAccount.bank_name?.replace('of the Philippines', '')} •••• {app.paymentAccount.account_number?.slice(-4)}
                              </span>
                            ) : (
                              <span className="inline-flex items-center gap-1 text-[9px] font-bold text-amber-800 bg-amber-100 border border-amber-300 px-2 py-0.5 rounded">
                                <AlertTriangle className="w-3 h-3 text-amber-700" /> Bank Proof Pending
                              </span>
                            )}
                          </div>
                        )}
              ...
              {processedScholars.length === 0 ? (
                <tr>
                  <td colSpan={7} className="px-6 py-12 text-center text-[#8E8E93]">
                    <div className="flex flex-col items-center justify-center gap-2">
                      <div className="w-12 h-12 bg-[#EDE8DE] rounded-full flex items-center justify-center text-[#2D5941] text-xl mb-1">
                        <GraduationCap className="w-6 h-6 text-[#2D5941]" />
                      </div>
                    ...
                    <div className="flex flex-col items-start gap-1.5">
                      <div className="flex items-center gap-1.5">
                        ...
                        {sch.disbursement_mode === 'in_person_cash' ? (
                          <span className="inline-flex items-center gap-1 text-[9px] font-semibold text-slate-600 bg-slate-100 px-2 py-0.5 rounded">
                            <Banknote className="w-3 h-3 text-slate-500" /> Cash
                          </span>
                        ) : sch.paymentAccount ? (
                          <span className="inline-flex items-center gap-1 text-[9px] font-bold text-[#2D5941] bg-[#EBF5EE] border border-[#2D5941]/30 px-2 py-0.5 rounded" title={`${sch.paymentAccount.bank_name} (${sch.paymentAccount.account_number})`}>
                            <CreditCard className="w-3 h-3 text-[#2D5941]" /> {sch.paymentAccount.bank_name?.replace('of the Philippines', '')} •••• {sch.paymentAccount.account_number?.slice(-4)}
                          </span>
                        ) : (
                          <span className="inline-flex items-center gap-1 text-[9px] font-bold text-amber-800 bg-amber-100 border border-amber-300 px-2 py-0.5 rounded">
                            <AlertTriangle className="w-3 h-3 text-amber-700" /> Bank Proof Missing
                          </span>
                        )}
                      </div>
                              ...
                              if (match) {
                                ...
                                if (isRefunded) {
                                  return (
                                    ...
                                    >
                                      <RotateCw className="w-2.5 h-2.5 text-rose-700" /> {displaySem} Payout: Refunded
                                    </span>
                                  );
                                }
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    )} {displaySem} Payout: {isReleased ? `₱${match.amount?.toLocaleString()} (Released)` : 'Pending Release'}
                                  </span>
                                );
                              } else {
                                return (
                                  ...
                                  >
                                    <Clock className="w-2.5 h-2.5 text-slate-400" /> {displaySem} Payout Pending
                                  </span>
                                );
                              }
                            })}
